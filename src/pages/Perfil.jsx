...
export default function Perfil() {
  const { user, setUserFromResponse, deleteAccount, updateProfile } = useAuth()
  ...
  const [ownPostsTab, setOwnPostsTab] = useState('posts')
  ...
  const renderSecaoPublicacoes = () => {
    ...
    const reelsPosts = Array.isArray(ownProfilePosts)
      ? ownProfilePosts.filter(p => p?.imageUrl && isVideoAttachment(p.imageUrl))
      : []
    ...
    return (
      <div className="bg-white rounded-2xl shadow p-3 sm:p-6">
        ...
        <div className="border-b border-gray-200 mb-4">
          <div className="grid grid-cols-2 py-2 text-xs font-semibold text-gray-600">
            <button
              type="button"
              onClick={() => setOwnPostsTab('posts')}
              className={`text-center py-2 rounded-lg transition ${ownPostsTab === 'posts' ? 'text-gray-900 bg-gray-50' : 'hover:bg-gray-50'}`}
            >
              PUBLICAÇÕES
            </button>
            <button
              type="button"
              onClick={() => setOwnPostsTab('reels')}
              className={`text-center py-2 rounded-lg transition ${ownPostsTab === 'reels' ? 'text-gray-900 bg-gray-50' : 'hover:bg-gray-50'}`}
            >
              REELS
            </button>
          </div>
      ...
      {activePostImageUrl ? (
        ...
        >
          ...
          >
            <div className="relative w-full h-full bg-black">
              {(() => {
                const raw = String(activePostImageUrl || '')
                const isVideo = raw && (raw.startsWith('data:video/') || /\.(mp4|webm|ogg)(\?|#|$)/i.test(raw))
                if (isVideo) {
                  return (
                    <video
